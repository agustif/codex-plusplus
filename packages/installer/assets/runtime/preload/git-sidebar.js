...
exports.startGitSidebar = startGitSidebar;
const electron_1 = require("electron");
const PROJECT_ROW_SELECTOR = "[data-app-action-sidebar-project-row][data-app-action-sidebar-project-id]";
const ACTIVE_THREAD_SELECTOR = "[data-app-action-sidebar-thread-active='true'],[data-app-action-sidebar-thread-active=true]";
...
async function refresh() {
    ...
    const activeProject = (activePath ? projects.find((project) => project.path === activePath) : null) ??
    ...
    const badgeStatuses = await Promise.all(badgeProjects.map(async (project) => {
    ...
    const summaryProject = badgeStatuses.find(({ project, status }) => project.path === activeProject?.path && isUsableRepo(status))
        ?.project ??
...
function getActiveProjectPath(projects) {
    ...
    if (listPath)
        return listPath;
    const expanded = projects.find((project) => project.row.getAttribute("data-app-action-sidebar-project-collapsed") === "false");
...
async function getStatus(path) {
    ...
    if (cached?.value && now - cached.loadedAt < STATUS_TTL_MS)
        return cached.value;
    if (cached?.pending)
    ...
    entry.pending = electron_1.ipcRenderer
...
async function getDetails(path) {
    ...
    if (cached?.value && now - cached.loadedAt < DETAILS_TTL_MS)
        return cached.value;
    if (cached?.pending)
        return cached.pending;
    const entry = cached ?? {
    ...
    entry.pending = Promise.all([
        electron_1.ipcRenderer.invoke("codexpp:git-diff-summary", path),
        electron_1.ipcRenderer.invoke("codexpp:git-worktrees", path),
    ])
        .then(([diff, worktrees]) => {
...
function renderSummaryPanel(project, status, details) {
    ...
    if (sync.short)
        title.append(textEl("span", sync.short));
    ...
    metrics.append(metric("staged", counts.staged), metric("unstaged", counts.unstaged), metric("untracked", counts.untracked), metric("conflicts", counts.conflicts));
    panel.append(metrics);
    if (diff) {
        const diffLine = el("div", "codexpp-git-summary-line");
        diffLine.append(textEl("span", `${diff.fileCount} file${plural(diff.fileCount)}`), textEl("span", `+${diff.insertions}`), textEl("span", `-${diff.deletions}`), ...(diff.truncated ? [textEl("span", "truncated")] : []));
    ...
    if (worktrees.length > 1) {
        ...
        for (const worktree of worktrees.slice(0, MAX_WORKTREE_ROWS)) {
            ...
            row.append(textEl("span", worktree.branch ?? shortSha(worktree.head) ?? "detached"), textEl("span", basename(worktree.path)));
...
//# sourceMappingURL=git-sidebar.js.map