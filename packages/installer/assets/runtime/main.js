...
function getWatcherHealth(userRoot2) {
  const checks = [];
  const state2 = readJson((0, import_node_path5.join)(userRoot2, "state.json"));
  const config = readJson((0, import_node_path5.join)(userRoot2, "config.json")) ?? {};
  checks.push({
    name: "Install state",
    status: state2 ? "ok" : "error",
    detail: state2 ? `Codex++ ${state2.version ?? "(unknown version)"}` : "state.json is missing"
  });
  if (!state2) return summarize("none", checks);
  ...
  checks.push({
    name: "Watcher kind",
    status: state2.watcher && state2.watcher !== "none" ? "ok" : "error",
    detail: state2.watcher ?? "none"
  });
  const appRoot = state2.appRoot ?? "";
  checks.push({
    name: "Codex app",
  ...
  return summarize(state2.watcher ?? "none", checks);
}
...
function inferCurrentChannel(userRoot2, state2, appName) {
  if (state2?.codexChannel === "stable" || state2?.codexChannel === "beta") return state2.codexChannel;
  const text = `${userRoot2} ${state2?.appRoot ?? ""} ${state2?.codexBundleId ?? ""} ${appName ?? ""}`;
...
async function readPatchChannelStatus(options) {
  ...
  const state2 = readJson2(statePath);
  ...
  const appRoot = state2?.appRoot ?? defaultAppRoot(options.channel, options.homeDir, options.platform);
  ...
  return {
    ...
    stateExists: state2 !== null,
    codexVersion: state2?.codexVersion ?? null,
    codexPlusPlusVersion: state2?.version ?? null,
    bundleId: state2?.codexBundleId ?? null,
    watcher: state2?.watcher ?? null,
    watcherLabel,
    ...
    runtimePreloadBytes,
    runtimeUpdatedAt: state2?.runtimeUpdatedAt ?? null,
...
// src/app-server-flow-tap.ts
var import_node_fs7 = require("node:fs");
var import_node_path7 = require("node:path");
var DEFAULT_MAX_FLOW_LOG_BYTES = 25 * 1024 * 1024;
var MAX_LINE_CHARS = 2e5;
var MAX_BUFFERED_CHARS = 1e6;
var MAX_QUEUED_LOG_BYTES = 2 * 1024 * 1024;
var state = {
  installed: false,
  enabled: false,
  source: "off",
...
function configureAppServerFlowTap(config) {
  ...
  try {
    (0, import_node_fs7.mkdirSync)((0, import_node_path7.dirname)(state.logPath), { recursive: true });
  } catch {
  }
  writeFlowEvent({
...
function isCodexAppServerSpawn(command, args) {
  ...
  const cmdBase = (0, import_node_path7.basename)(cmd).toLowerCase();
...
function summarizeJsonRpcLine(line) {
  ...
  let kind = "unknown";
  if (method && parsed.id !== void 0) kind = "request";
  else if (method) kind = "notification";
  else if (error) kind = "error";
  else if (parsed.id !== void 0 && parsed.result !== void 0) kind = "response";
  return {
    kind,
    ...parsed.id !== void 0 ? { id: scalarId(parsed.id) } : {},
    ...method ? { method } : {},
...
function correlateJsonRpcSummary(childKey, stream, summary) {
  if (!summary || summary.id === void 0 || summary.id === null) return summary;
...
function prunePendingRequests() {
  while (state.pendingRequests.size > 2e3) {
...
var APP_SERVER_FLOW_TAP_LOG_FILE = (0, import_node_path8.join)(LOG_DIR, "app-server-flow.jsonl");
var CONFIG_FILE = (0, import_node_path8.join)(userRoot, "config.json");
var CODEX_CONFIG_FILE = (0, import_node_path8.join)((0, import_node_os3.homedir)(), ".codex", "config.toml");
...
(0, import_node_fs8.mkdirSync)(TWEAKS_DIR, { recursive: true });
...
var startupFlowTap = resolveStartupAppServerFlowTapConfig();
...
function readState() {
  try {
    return JSON.parse((0, import_node_fs8.readFileSync)(CONFIG_FILE, "utf8"));
  } catch {
    return {};
  }
}
function writeState(s) {
  try {
    (0, import_node_fs8.writeFileSync)(CONFIG_FILE, JSON.stringify(s, null, 2));
  } catch (e) {
...
function setAppServerFlowTapConfig(config) {
  const s = readState();
...
function readInstallerState() {
  try {
    return JSON.parse((0, import_node_fs8.readFileSync)(INSTALLER_STATE_FILE, "utf8"));
  } catch {
    return null;
  }
}
function readEarlyCdpConfig() {
  try {
    const parsed = JSON.parse((0, import_node_fs8.readFileSync)(CONFIG_FILE, "utf8"));
    return parsed.codexPlusPlus?.cdp ?? {};
  } catch {
    return {};
  }
}
function readEarlyAppServerFlowTapConfig() {
  try {
    const parsed = JSON.parse((0, import_node_fs8.readFileSync)(CONFIG_FILE, "utf8"));
...
function resolveStartupAppServerFlowTapConfig() {
...
function getCodexCdpStatus() {
  ...
  const configured = state2.codexPlusPlus?.cdp ?? {};
...
function getAppServerFlowTapStatus() {
  return {
...
function readFileSize(path) {
  try {
    return (0, import_node_fs8.statSync)(path).size;
  } catch {
    return null;
  }
}
function readFileTail(path, maxBytes) {
  try {
    ...
    const data = (0, import_node_fs8.readFileSync)(path);
...
function prepareSignedCodexForSparkleInstall() {
  if (process.platform !== "darwin") return;
  if ((0, import_node_fs8.existsSync)(UPDATE_MODE_FILE)) {
    log("info", "Sparkle update prep skipped; update mode already active");
    return;
  }
  if (!(0, import_node_fs8.existsSync)(SIGNED_CODEX_BACKUP)) {
  ...
  const state2 = readInstallerState();
  const appRoot = state2?.appRoot ?? inferMacAppRoot();
  if (!appRoot) {
  ...
  (0, import_node_fs8.writeFileSync)(UPDATE_MODE_FILE, JSON.stringify(mode, null, 2));
  try {
...
import_electron.ipcMain.handle("codexpp:list-tweaks", async () => {
  ...
  return tweakState.discovered.map((t) => ({
    ...
    entry: t.entry,
    dir: t.dir,
    entryExists: (0, import_node_fs8.existsSync)(t.entry),
...
import_electron.ipcMain.handle("codexpp:get-app-server-flow-tap-status", () => getAppServerFlowTapStatus());
import_electron.ipcMain.handle("codexpp:set-app-server-flow-tap-config", (_e, config) => {
...
import_electron.ipcMain.handle("codexpp:tweak-fs", (_e, op, id, p, c) => {
  ...
  const full = (0, import_node_path8.join)(dir, p);
  const fs = require("node:fs");
  switch (op) {
    case "read":
...
import_electron.ipcMain.handle("codexpp:read-app-server-flow-tap-log", (_e, maxBytes) => {
  const normalizedMaxBytes = typeof maxBytes === "number" && Number.isFinite(maxBytes) ? Math.max(1, Math.min(Math.floor(maxBytes), 1024 * 1024)) : 192 * 1024;
  return readFileTail(APP_SERVER_FLOW_TAP_LOG_FILE, normalizedMaxBytes);
});
import_electron.ipcMain.handle("codexpp:open-app-server-flow-tap-log", () => {
  import_electron.shell.openPath(APP_SERVER_FLOW_TAP_LOG_FILE).catch(() => {
  });
});
import_electron.ipcMain.handle("codexpp:reveal-app-server-flow-tap-log", () => {
  import_electron.shell.openPath(LOG_DIR).catch(() => {
  });
});
...
var RUNTIME_PRELOAD_RELOAD_DEBOUNCE_MS = 180;
var RUNTIME_PRELOAD_RELOAD_MARKER = (0, import_node_path8.join)(runtimeDir, ".codexpp-runtime-reload");
...
var runtimePreloadReloadTimer = null;
function scheduleRuntimePreloadReload(reason) {
...
function startRuntimePreloadWatcher() {
  try {
    const watcher = esm_default.watch([PRELOAD_PATH, RUNTIME_PRELOAD_RELOAD_MARKER], {
      ignoreInitial: true,
      awaitWriteFinish: { stabilityThreshold: 150, pollInterval: 50 }
    });
...
function reloadRendererWindows(reason) {
  let count = 0;
...
async function ensureCodexPlusPlusUpdateCheck(force = false) {
  const state2 = readState();
  const cached = state2.codexPlusPlus?.updateCheck;
  ...
  state2.codexPlusPlus ??= {};
  state2.codexPlusPlus.updateCheck = check;
  writeState(state2);
  return check;
}
async function ensureTweakUpdateCheck(t) {
  ...
  const state2 = readState();
  const cached = state2.tweakUpdateChecks?.[id];
  ...
  state2.tweakUpdateChecks ??= {};
  state2.tweakUpdateChecks[id] = check;
  writeState(state2);
}
...
*/
//# sourceMappingURL=data:application/json;base64,
